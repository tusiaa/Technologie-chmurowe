...
function App() {
  ...
  const onDelete = () => { 
    if (DoUsuniecia.length !== 0) {
      ...
      setSzukaneKsiazki( szukaneksiazki.filter(ksiazka => 
        DoUsuniecia.map(a => ksiazka._id!==a).reduce((a,b) => a && b) ) )
  ...
  const onDelete2 = (id) => { 
    ...
    setKsiazki( ksiazki.filter(ksiazka => ksiazka._id!==id))
    setAddEdit(false) 
  }  
  ...
  const toSearch = (wymagania) => {
    setSzukaneKsiazki( ksiazki
      ...
      .filter(ksiazka => Ulubione.includes(ksiazka._id) || wymagania[4]=== "" )
    )
    setPage(1)
  }
      ...
      {!isError && !isLoading && <Router>
        <Switch>
          ...
          <Route exact path="/book/:id">
            <NavBar2 onDelete={onDelete2} onEdit={onAdd} setAdd={setAddEdit} onLogin={onLogin} onLogout={onLogout} Login={MainLogin} />
          ...
          <Route exact path="/">
            <NavBar onDelete={onDelete} onAdd={onAdd} onSearch={onSearch} ChangeSort={ChangeSort} setAdd={setAddEdit} setSearch={setSearch} onLogin={onLogin} onLogout={onLogout} Login={MainLogin} />

            {Login && <LoginRegisterForm setLogin2={setLogin} setMainLogin={setMainLogin} />}
            {AddEdit && <AddEditForm ksiazki = {ksiazki} setAdd={setAddEdit} />}
            {Search && <SearchForm Search={toSearch} gatunki={ksiazki.map(a => a.genre).reduce(UsuwaniePowtorzen, [])} />}

            {MainLogin && <LoginInfo Login={MainLogin} setLogin={setLogin} setMainLogin={setMainLogin} />}
